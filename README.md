...
- Entra em **Área Reservada** → faz login. A tua conta de administrador:
  - **Email:** `elmar.bravo@barispol.com`
  - **Senha:** `B@rispol2022`
- Abre **Gestão de Acessos**: adiciona/remove colaboradores, define o **perfil** e a **senha** de cada um (e cada colaborador pode alterar a sua própria senha no portal).
- O **Workspace** (intranet) também exige login — valida contra a mesma lista de utilizadores. Já não entra com qualquer email.
...
⚠️ **Importante:** neste protótipo, os utilizadores/senhas e as campanhas ficam guardados **no navegador** (localStorage) de cada dispositivo — partilhados entre o site e o Workspace por estarem no mesmo domínio. Serve para demonstrar o funcionamento; **não é ainda uma autenticação real centralizada**. Recomenda-se trocar as senhas de exemplo (1234) e, em produção, ligar ao Microsoft Entra ID.
